...
| 2026-05-27 | Déplacement RPC `check_email_status` et `check_email_role` vers schema `private` + Edge Function rate limiting | Migration `20260527030000_email_check_functions_to_private_schema.sql` : functions déplacées en `private`, Edge Function `check-email-availability` implémentée avec rate limiting (10 req/60s par IP), clients mis à jour via `auth-helpers.ts`. Warnings Advisor 0028/0029 (SECURITY DEFINER) éliminés. | ✅ Corrigé |
| 2026-05-27 | HaveIBeenPwned protection désactivée (limitation Supabase free plan) | Documenté comme OPTION B (risque accepté). `search_path` sécurisé côté auth. Implémentation Edge Function planifiée pour futur (nécessite Supabase Pro). | ⚠️ Risque accepté — planifié |
